...
public record AddBookingCommand(int trainId,int userId,BookingRequestDTO bookingRequestDTO):IRequest<PassengerBookingInfoDTO>;
...
public async Task<PassengerBookingInfoDTO> Handle(AddBookingCommand request, CancellationToken cancellationToken)
{
    ...
    try
    {
        var bookingDetails = request.bookingRequestDTO;
        ...
        await emailNotificationService.SendBookingConfirmationAsync(bookingInfo,completeBooking);
